...
BinaryNode<ItemType>* BinarySearchTree<ItemType>::findNode(BinaryNode<ItemType>* subTreePtr,
	const ItemType& target, int& count) const
{
	...
	if (subTreePtr == nullptr) {
		...
		// Search left subtree
		count++;
		return findNode(subTreePtr->getLeftChildPtr(), target, count);
	}
	else {
		// Search right subtree
		count++;
		return findNode(subTreePtr->getRightChildPtr(), target, count);
	}
}  // end findNode
...
{
	int count = 0;
	BinaryNode<ItemType>* nodeWithEntry = findNode(rootPtr, anEntry, count);
...
template<class ItemType>
bool BinarySearchTree<ItemType>::contains(const ItemType& anEntry, int& count) const
{
	return (findNode(rootPtr, anEntry, count) != nullptr) ? true : false;  // nullptr is same as false
}  // end contains
